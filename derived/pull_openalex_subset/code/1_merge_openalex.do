...
cd "$github/hpc/derived/pull_openalex_subset/code"

global min_year = 2007
global max_year = 2024 
...
program append_files
    ...
    save "${derived_output}/pull_openalex_subset/openalex_all_jrnls_merged", replace
    preserve
	gcontract id, nomiss
	drop _freq
	save "${derived_output}/pull_openalex_subset/list_of_works", replace
	restore
	preserve
	gcontract athr_id, nomiss
	drop _freq
	save "${derived_output}/pull_openalex_subset/list_of_athrs", replace
	restore
    gcontract inst_id, nomiss
    drop _freq
    save "${derived_output}/pull_openalex_subset/list_of_insts", replace
end
program append_mesh
        forval i = $min_year / $max_year {
        di "`i'"
		...
		merge m:1 id using "${derived_output}/pull_openalex_subset/list_of_works", assert(1 2 3) keep(3) nogen
        save "${derived_output}/pull_openalex_subset/contracted_gen_mesh_all_jrnls", replace
end
program append_concepts
		...
		merge m:1 id using "${derived_output}/pull_openalex_subset/list_of_works", assert(1 2 3) keep(3) nogen
        save "${derived_output}/pull_openalex_subset/topics_all_jrnls_merged",replace
end
program append_grants
		...
		merge m:1 id using "${derived_output}/pull_openalex_subset/list_of_works", assert(1 2 3) keep(3) nogen
        save "${derived_output}/pull_openalex_subset/grants_all_jrnls_merged",replace
end
main
